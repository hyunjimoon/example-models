...
ss <- extract(fit)
df <- data.frame(list(lynx = ss$z[1, 1:12 , 2], hare = ss$z[1, 1:12, 1], draw = 1))
for (m in 2:100) {
  df <- rbind(df, data.frame(list(lynx = ss$z[m, 1:12 , 2], hare = ss$z[m, 1:12, 1], draw = m)))
}
plot <- ggplot(df) +
  geom_vline(xintercept = 0, color = "grey") +
  geom_hline(yintercept = 0, color = "grey") +
...
ss <- extract(fit)
df <- data.frame(list(lynx = ss$y_rep[1, 1:12 , 2], hare = ss$y_rep[1, 1:12, 1], draw = 1))
for (m in 2:100) {
  df <- rbind(df, data.frame(list(lynx = ss$y_rep[m, 1:12 , 2], hare = ss$y_rep[m, 1:12, 1], draw = m)))
}
plot <- ggplot(df) +
  geom_vline(xintercept = 0, color = "grey") +
  geom_hline(yintercept = 0, color = "grey") +
